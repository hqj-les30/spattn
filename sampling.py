...
def get_mixed_noniid(dataset, num_users, ratio, num_items=2):
    """
    Sample mixed non-I.I.D client data from dataset
    :param dataset:
    :param num_users:
    :non-i.i.d ratio
    :param num_items: number of shards per user (fewer = more heterogeneous)
    :return:
    """
    # iid sample
    ...
    for i in range(num_users):
        if iidnum_items > 0:
            ...
            dict_users[i] = np.ones(shape=(0,), dtype=int)
...
if __name__ == '__main__':
    ...
    num = 50
    d = get_mixed_noniid(train_dataset, num, 0.95)
    # utils.dataset_stats(d, train_dataset)